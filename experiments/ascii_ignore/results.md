...
Let $T_c := $ UDF compute time.  
Let $T_s := $ round-trip serialization time.  
    - 1 round trip: `writeArrowIPCArrowChunk + readArrowIPCChunkToArrowTable`
Let $T_p := $ round-trip PCIe transfer time. We decompose $T_p$ into two hops:
...
Let $T^{(n)} := $ wall-clock time of the entire UDF op (including data transfer) with $n$ threads.
