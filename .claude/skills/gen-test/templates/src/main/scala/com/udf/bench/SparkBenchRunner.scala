 ...
 *   Read Parquet → Execute (CPU or GPU) → Write Parquet
 *
...
object SparkBenchRunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // UDF outputs are written to a temp output directory (under CWD).
    // This will be deleted after execution.
    val tmpOutputDir = Files.createTempDirectory(
      Paths.get("."), "bench_output_").toFile
    ...
    try {
      ...
      resultDf.write.mode("overwrite").parquet(tmpOutputDir.getAbsolutePath)
      ...
      spark.stop()
      deleteRecursively(tmpOutputDir)
    }
  }
